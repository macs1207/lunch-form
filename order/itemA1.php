	...
	if(traget.style.display=="none"){
		...
		<!-- Header -->
			<section id="header">
				...
				<nav id="nav">
					<ul>
						<li><a href="/order/itemA1.php" class="active">第一週</a></li>
						<li><a href="/order/itemA2.php">第二週</a></li>
						<li><a href="/order/itemA3.php">第三週</a></li>
						<li><a href="/order/itemA4.php">第四週</a></li>
					</ul>
				</nav>
				<footer>
                	<li><a href="/order/userA1.php" class="button fit">用戶排列</a></li>
											...
											for ($i = 0; $i <= $cnt; $i++){
												if ($order[$i] != "") {		
													...
													if (mysqli_query($conn, $sql)) {
														...
														echo "window.location.href='/order/itemA$week.php'";
														echo "</script>";
													} else {
								...
								<div class="container">
									<header class="major">
										...
										<p>
											...
											$day = "3";
											$price = 0;
											$sql1 = "SELECT * FROM user1 WHERE class='1'";
											$result1 = $conn->query($sql1);
											if ($result1->num_rows > 0) {
											    while($row1 = $result1->fetch_assoc()) {
													$sql = "SELECT * FROM week$week WHERE day='".$day."' AND userid='".$row1["no"]."'";
											...
											$sql = "SELECT * FROM week$week WHERE day='".$day."' AND class='1' ORDER BY name";
											$result = $conn->query($sql);
											if ($result->num_rows > 0) {
										    	// output data of each row
												echo("<label>已點餐點</label>");
